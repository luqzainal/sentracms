-- Migration: Add deadline fields for parent items (Onboarding, First Draft, Second Draft, Handover)
-- Date: 2024-12-19
...
ADD COLUMN IF NOT EXISTS handover_completed_date DATE;
...
SET 
  ...
  handover_deadline = deadline + INTERVAL '28 days'
WHERE title LIKE '% - Package Setup%';

-- Create index for better performance
CREATE INDEX IF NOT EXISTS idx_progress_steps_deadlines 
ON progress_steps (onboarding_deadline, first_draft_deadline, second_draft_deadline, handover_deadline);
...
COMMENT ON COLUMN progress_steps.handover_deadline IS 'Deadline for project handover phase';
...
COMMENT ON COLUMN progress_steps.handover_completed IS 'Whether project handover is completed'; 