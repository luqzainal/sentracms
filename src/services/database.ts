      ...
      const stepsWithComments = await Promise.all(
        data.map(async (row) => {
          ...
          return {
            ...
            handover_deadline: row.handover_deadline,
            ...
            handover_completed_date: row.handover_completed_date,
            created_at: row.created_at,
            updated_at: row.updated_at,
            client: row.client_name ? {
              id: row.client_id,
    ...
    try {
      const data = await sql!`
        ...
        SET 
          ...
          handover_deadline = COALESCE(${updates.handover_deadline}, handover_deadline),
          ...
          handover_completed = COALESCE(${updates.handover_completed}, handover_completed),
          ...
          handover_completed_date = COALESCE(${updates.handover_completed_date}, handover_completed_date),
          updated_at = NOW()
        WHERE id = ${id}
        RETURNING *
      `;
